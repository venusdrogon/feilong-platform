...
public class TransactionAttributeUtil{
    ...
    /** The Constant log. */
    private static final Logger log = LoggerFactory.getLogger(TransactionAttributeUtil.class);
    ...
    private static String toPropagationBehaviorString(int propagationBehavior){
        ...
        //默认的事务传播行为，表示必须有逻辑事务，否则新建一个事务
            case TransactionDefinition.PROPAGATION_REQUIRED:
                propagationBehaviorString = "PROPAGATION_REQUIRED";
                break;

            //创建新的逻辑事务,表示每次都创建新的逻辑事务（物理事务也是不同的）,因此外部事务可以不受内部事务回滚状态的影响独立提交或者回滚。
            case TransactionDefinition.PROPAGATION_REQUIRES_NEW:
                propagationBehaviorString = "PROPAGATION_REQUIRES_NEW";
                break;

            //指如果当前存在逻辑事务，就加入到该逻辑事务， 如果当前没有逻辑事务，就以非事务方式执行。
            case TransactionDefinition.PROPAGATION_SUPPORTS:
                propagationBehaviorString = "PROPAGATION_SUPPORTS";
                break;

            //不支持事务，如果当前存在事务则暂停该事务,如果当前存在逻辑事务，就把当前事务暂停，以非事务方式执行
            case TransactionDefinition.PROPAGATION_NOT_SUPPORTED:
                propagationBehaviorString = "PROPAGATION_NOT_SUPPORTED";
                break;

            //如果当前有事务，使用当前事务执行，如果当前没有事务，则抛出异常（IllegalTransactionStateException）
            case TransactionDefinition.PROPAGATION_MANDATORY:
                propagationBehaviorString = "PROPAGATION_MANDATORY";
                break;

            //不支持事务，  如果当前存在是事务则抛出IllegalTransactionStateException异常，
            case TransactionDefinition.PROPAGATION_NEVER:
                propagationBehaviorString = "PROPAGATION_NEVER";
                break;

            //嵌套事务支持
            case TransactionDefinition.PROPAGATION_NESTED:
                propagationBehaviorString = "PROPAGATION_NESTED";
                break;
