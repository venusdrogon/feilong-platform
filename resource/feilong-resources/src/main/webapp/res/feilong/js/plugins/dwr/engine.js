/*
 * Copyright 2005 Joe Walker Licensed under the Apache License, Version 2.0 (the "License"); you may not use this file except in compliance with the License.
 ...
 * 
 * @param newType
 *          One of dwr.engine.XMLHttpRequest or dwr.engine.IFrame or dwr.engine.ScriptTag
 ...
 * 
 * @param newPollType
 *          One of dwr.engine.XMLHttpRequest or dwr.engine.IFrame
 * @see getahead.org/dwr/browser/engine/options
 */
...
dwr.engine.endBatch = function(options) {
	...
	if (options)
		dwr.engine._mergeBatch(batch, options);
...
dwr.engine._origScriptSessionId = "5AF61FBADBA900F3C7CEC04CA68CC1D7";
...
dwr.engine._defaultPath = "/epai/dwr";
...
dwr.engine._DOMDocument = ["Msxml2.DOMDocument.6.0", "Msxml2.DOMDocument.5.0", "Msxml2.DOMDocument.4.0", "Msxml2.DOMDocument.3.0", "MSXML2.DOMDocument",
 ...
 * @param path
 *          part of URL after the host and before the exec bit without leading or trailing /s
 * @param scriptName
 *          The class to execute
 * @param methodName
 *          The method on said class to execute
 * @param func
 *          The callback function to which any returned data should be passed if this is null, any returned data will be ignored
 * @param vararg_params
...
dwr.engine._sendData = function(batch) {
	...
	if (batch.req) {
		...
		batch.div.innerHTML = "<iframe src='javascript:void(0)' frameborder='0' style='width:0px;height:0px;border:0;' id='" + idname + "' name='" + idname
		+ "'></iframe>";
		...
		if (batch.isPoll)
			dwr.engine._outstandingIFrames.push(batch.iframe);
		request = dwr.engine._constructRequest(batch);
...
dwr.engine._clearUp = function(batch) {
	...
	if (batch.req) {
		// If this is a poll frame then stop comet polling
		if (batch.req == dwr.engine._pollReq)
			dwr.engine._pollReq = null;
		delete batch.req;
	}
	...
	switch (typeof data) {
		...
		case "object" :
			if (data instanceof String)
				...
				batch.map[name] = dwr.engine._serializeArray(batch, referto, data, name);
			else
...
dwr.engine._getObjectClassName = function(obj) {
	...
	if (obj && obj.constructor) {
		for (var errorname in dwr.engine._errorClasses) {
			if (obj.constructor == dwr.engine._errorClasses[errorname])
				return errorname;
		}
	}
