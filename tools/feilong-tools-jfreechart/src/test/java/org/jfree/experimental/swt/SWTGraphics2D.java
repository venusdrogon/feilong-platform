...
public class SWTGraphics2D extends Graphics2D{
	...
	private static final Logger	log				= LoggerFactory.getLogger(SWTGraphics2D.class);
	 ...
	 * context only supports instances of {@link Color} or {@link GradientPaint} (in the latter case there is no real gradient
	...
	public void setComposite(Composite comp){
		...
		if (comp instanceof AlphaComposite){
			...
			log.info("warning, can only handle alpha composite at the moment.");
		}
	}
	 ...
	 * The <code>fillRectangle</code> method of the <code>GC</code> class uses the background color so we must switch colors.
	 * 
	...
	/**
	 * Not implemented - see {@link Graphics#drawImage(Image, int, int, int, int, int, int, int, int, Color, ImageObserver)}.
	 * 
	 * @param image
	 *            the image.
	 * @param dx1
	...
	private org.eclipse.swt.graphics.Font getSwtFontFromPool(Font font){
		...
		if (swtFont == null){
			swtFont = new org.eclipse.swt.graphics.Font(this.gc.getDevice(), SWTUtils.toSwtFontData(this.gc.getDevice(), font, true));
